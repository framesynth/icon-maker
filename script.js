...
// ================================
let pointerState = { pointers: new Map(), isDragging: false, lastX: 0, lastY: 0, lastDist: 0 };
...
// ================================
// ▼ Drawing process（はみ出し禁止／余白なし切替）
// ================================
function redraw(fillFrame = false) {
  ...
  if (baseImage) {
    ...
    const autoScale = fillFrame ? Math.max(fw / iw, fh / ih) : Math.min(fw / iw, fh / ih);
    const appliedScale = Math.max(scale, autoScale);
    ...
    // はみ出し禁止の境界チェック
    if (!fillFrame) {
      if (dx > fw - dw) dx = fw - dw;
      if (dx < 0) dx = 0;
      if (dy > fh - dh) dy = fh - dh;
      if (dy < 0) dy = 0;
    }
